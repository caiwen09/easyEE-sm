...
<script type="text/javascript">
	...
	$(function() {
		...
		function toEdit(row) {
			...
			//查询角色所有权限
			$.post("SysRole/getAllPermissionsId",
				   "roleId="+ row.roleId, function(data) {
				/*
				 * 初始化权限复选框
				 */
	...
	<%-- 2. 页面内容 --%>
	<table id="sysRoleDataGrid" title="角色列表" style="width: 100%" toolbar="#sysRoleToolbar" idField="roleId" rownumbers="true" fitColumns="true" nowrap="false">
		<thead>
			<tr>
				<th field="ck" checkbox="true" width="50" sortable="true">多选框</th>
				<th field="name" width="50" sortable="true">名称</th>
				<th field="status" width="50" formatter="sysRole.formatStatus" sortable="true">状态</th>
	...
	<div id="sysRoleToolbar">
		<%--搜索框--%>
		<div>
			<span>角色名:</span> <input name="name" id="sysRoleName" class="easyui-textbox" /> <span>状态:</span>
			<select name="status" class="easyui-combobox" id="sysRoleStatus" style="width:90px;" data-options="editable:false,panelHeight:'auto'">
				<option value="">--全部--</option>
				<option value="0">正常</option>
				<option value="1">禁用</option>
			</select>
			<a class="easyui-linkbutton" iconCls="icon-search" plain="true" onclick="sysRole.doSearch()"><spring:message code="label.search"></spring:message></a>
		</div>

		<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" onclick="sysRole.toAdd()">添加角色</a>
		<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-edit',plain:true" onclick="sysRole.toEdit()">修改角色</a>
		<a class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="sysRole.toDelete()">删除角色</a>
	</div>
