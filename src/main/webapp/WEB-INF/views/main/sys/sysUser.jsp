...
<script type="text/javascript">
	//用户操作命名空间,防止命名重复，最好使用sysUser.作为前缀
	var sysUser = {};
	$(function() {
		/*
		 * datagrid数据格式化
...
<%-- 2. 页面内容 --%>
<table id="sysUserDataGrid" title="用户列表" toolbar="#sysUserToolbar" idField="userId" rownumbers="true" fitColumns="true" nowrap="false" fit="true" >
	<thead>
		<tr>
			...
			<th field="status" width="50" formatter="sysUser.formatStatus" sortable="true">状态</th>
...
<div id="sysUserToolbar">
	...
	<div>
		...
		<span>状态:</span>
			<select name="status" class="easyui-combobox" id="sysUserStatus" style="width:90px;" data-options="editable:false,panelHeight:'auto'">
				<option value="">--全部--</option>
				<option value="0">正常</option>
				<option value="1">禁用</option>
			</select>
		<a class="easyui-linkbutton" iconCls="icon-search" plain="true" onclick="sysUser.doSearch()"><spring:message code="label.search"></spring:message></a>
	</div>

	<%--操作菜单--%>
	<shiro:hasPermission name="sysUserAddBtn">
		<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true" onclick="sysUser.toAdd()">添加用户</a>
	</shiro:hasPermission>
	<shiro:hasPermission name="sysUserUpdateBtn">
		<a href="javascript:void(0)" class="easyui-linkbutton" data-options="iconCls:'icon-edit',plain:true" onclick="sysUser.toEdit()">修改用户</a>
	</shiro:hasPermission>
	<shiro:hasPermission name="sysUserDelBtn">
		<a class="easyui-linkbutton" iconCls="icon-remove" plain="true" onclick="sysUser.toDelete()">删除用户</a>
	</shiro:hasPermission>
